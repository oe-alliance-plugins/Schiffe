...
# Adapted from Lululla for WQHD 20260726
# ===============================================================================
...
def getSkinResolution():
	"""Return the current skin resolution (width, height) from the framebuffer."""
	s = getDesktop(0).size()
	return (s.width(), s.height())


def isSkinHD():
	w, h = getSkinResolution()
...
def Plugins(**kwargs):
	return [PluginDescriptor(name="Battleship", description=_("Battleship Game"), where=[PluginDescriptor.WHERE_PLUGINMENU],
...
class GameCell:
	...
	def paint(self):
		...
		if self.value_ == 2:
			# Use dynamic font size based on cell size
			font_size = max(18, min(36, int(self.w * 0.6)))
			self.canvas.writeText(self.x, self.y, self.w, self.h, fg, bg, gFont("Regular", font_size), '*', RT_HALIGN_CENTER | RT_VALIGN_CENTER)
...
class Schiffe(Screen):
	def __init__(self, session):
		...
		# ---- Calculate CELL_SIZE based on resolution ----
		if w <= 1280:
			CELL_SIZE = 40
			FONT_SIZE = 21
			# BUTTON_SIZE = 40
		...
		# ---- Calculate skin dimensions dynamically ----
		CELL_OFFSET = 2
		cellfield = XMAX * CELL_SIZE + (XMAX - 1) * CELL_OFFSET
		CW = 2 * cellfield + 150   # canvas width
		CH = cellfield             # canvas height
		X0_OFFSET = 0              # xoffset cellfield box
		# X1_OFFSET = cellfield + 150  # xoffset cellfield you

		if isSkinWQHD():
			X1_OFFSET = cellfield + 200
		# elif isSkinUHD():
			# X1_OFFSET = cellfield + 350
		else:
			X1_OFFSET = cellfield + 150

		W = CW + 10                # window width
		H = CH + 40                # window height
		...
		# ---- Build dynamic skin (generic) ----
		self.skin = """
			<screen position="center,center" size="%d,%d" title="Schiffe v.%s" >
				<widget source="Canvas" render="Canvas" position="5,20" size="%d,%d" />
				<widget name="message" position="%d,%d" size="140,40" valign="center" halign="center" font="Regular;%d" />
			...
			</screen>""" % (W, H, __version__, CW, CH, WX, W0Y, FONT_SIZE, WX, W1Y, WX, W2Y, WX, W3Y, WX, W1Y, FONT_SIZE, WX, W2Y, FONT_SIZE, WX, W3Y, FONT_SIZE)

		# ---- Use optimized FHD / WQHD / UHD skin if supported ----
		if isSkinFHD():
			self.skin = """
				<screen name="Schiffe" position="center,140" size="1800,900" title="Schiffe" backgroundColor="#101010">
					...
					<ePixmap position="1050,170" size="130,400" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/Schiffe/pic/ship.jpg" zPosition="5" scale="1" />
					...
					<ePixmap position="48,332" size="80,80" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/Schiffe/pic/rocket.png" alphatest="blend" zPosition="1" scale="1" />
			...
			# WQHD skin adapted from FHD with scaling factor 2560/1920 = 1.333
			self.skin = """
				<screen name="Schiffe" position="center,140" size="2400,1200" title="Schiffe" backgroundColor="#101010">
					<ePixmap position="0,0" size="2400,1200" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/Schiffe/pic/Schiffe.jpg" scale="1" />
					<ePixmap position="1390,280" size="173,533" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/Schiffe/pic/ship.jpg" zPosition="5" scale="1" />
					<widget name="message" position="66,13" size="466,93" valign="center" halign="center" font="Regular;53" foregroundColor="yellow" backgroundColor="#000000" transparent="1" zPosition="1" />
					<widget source="Canvas" render="Canvas" position="660,175" size="1635,745" backgroundColor="#60ffffff" transparent="1" alphatest="blend" zPosition="2" />
					<ePixmap position="66,200" pixmap="buttons/key_green.png" size="106,53" alphatest="blend" zPosition="2" />
					<widget name="key_green" font="Regular;40" position="180,200" size="600,53" halign="left" valign="center" backgroundColor="black" zPosition="1" transparent="1" />
					<ePixmap position="66,266" pixmap="buttons/key_red.png" size="106,53" alphatest="blend" zPosition="2" />
					<widget name="key_red" font="Regular;40" position="180,266" size="600,53" halign="left" valign="center" backgroundColor="black" zPosition="1" transparent="1" />
					<ePixmap position="66,333" pixmap="buttons/key_blue.png" size="106,53" alphatest="blend" zPosition="2" />
					<widget name="key_blue" font="Regular;40" position="180,333" size="600,53" halign="left" valign="center" backgroundColor="black" zPosition="1" transparent="1" />
					<eLabel position="66,400" size="400,4" backgroundColor="#202020" zPosition="1" />
					<ePixmap position="64,442" size="106,106" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/Schiffe/pic/rocket.png" scale="1" alphatest="blend" zPosition="1" />
					<widget name="result" render="Label" position="174,446" size="266,45" font="Regular;40" halign="left" foregroundColor="yellow" backgroundColor="#000000" transparent="1" zPosition="3" />
					<widget name="movex" render="Label" position="173,500" size="266,45" font="Regular;40" halign="left" foregroundColor="yellow" backgroundColor="#000000" transparent="1" zPosition="3" />
				</screen>"""
		elif isSkinUHD():
			# UHD skin adapted from FHD with scaling factor 3840/1920 = 2.0
			self.skin = """
				<screen name="Schiffe" position="center,140" size="3600,1800" title="Schiffe" backgroundColor="#101010">
					<ePixmap position="0,0" size="3600,1800" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/Schiffe/pic/Schiffe.jpg" scale="1" />
					<ePixmap position="2100,340" size="260,800" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/Schiffe/pic/ship.jpg" zPosition="5" scale="1" />
					<widget name="message" position="100,20" size="700,140" valign="center" halign="center" font="Regular;80" foregroundColor="yellow" backgroundColor="#000000" transparent="1" zPosition="1" />
					<widget source="Canvas" render="Canvas" position="1040,300" size="2400,1100" backgroundColor="#60ffffff" transparent="1" alphatest="blend" zPosition="2" />
					<ePixmap position="100,300" pixmap="buttons/key_green.png" size="160,80" alphatest="blend" zPosition="2" />
					<widget name="key_green" font="Regular;60" position="270,300" size="900,80" halign="left" valign="center" backgroundColor="black" zPosition="1" transparent="1" />
					<ePixmap position="100,400" pixmap="buttons/key_red.png" size="160,80" alphatest="blend" zPosition="2" />
					<widget name="key_red" font="Regular;60" position="270,400" size="900,80" halign="left" valign="center" backgroundColor="black" zPosition="1" transparent="1" />
					<ePixmap position="100,500" pixmap="buttons/key_blue.png" size="160,80" alphatest="blend" zPosition="2" />
					<widget name="key_blue" font="Regular;60" position="270,500" size="900,80" halign="left" valign="center" backgroundColor="black" zPosition="1" transparent="1" />
					<eLabel position="100,600" size="600,6" backgroundColor="#202020" zPosition="1" />
					<ePixmap position="96,664" size="160,160" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/Schiffe/pic/rocket.png" alphatest="blend" zPosition="1" scale="1" />
					<widget name="result" render="Label" position="262,670" size="400,68" font="Regular;60" halign="left" foregroundColor="yellow" backgroundColor="#000000" transparent="1" zPosition="3" />
					<widget name="movex" render="Label" position="260,750" size="400,68" font="Regular;60" halign="left" foregroundColor="yellow" backgroundColor="#000000" transparent="1" zPosition="3" />
				</screen>"""
		...
		for c in actualSkin.findall("colors"):
			for color in c.findall("color"):
				...
				name = get_attr("name")
				color_val = get_attr("value")
				if name and color_val:
					colorNames[name] = color_val
		...
		for windowstyle in actualSkin.findall("windowstyle"):
			if windowstyle.attrib.get("id") == "0":
				for color in windowstyle.findall("color"):
					get_attr = color.attrib.get
					type_name = get_attr("name")
					color_val = get_attr("color")
					if color_val and color_val[0] != '#':
						color_val = colorNames.get(color_val, "#000000")
					if type_name == "Background":
						bgcolor = int(color_val[1:], 0x10) if color_val and color_val[0] == '#' else RGB(0, 0, 0)
		if bgcolor is None:
			bgcolor = RGB(0, 0, 0)
		...
		self.setTitle("Schiffe v.%s" % __version__)
	...
	def timerHandler(self):
		if isSkinFHD() or isSkinWQHD() or isSkinUHD():
			self["result"].setText(_("%10d shots") % self.moves)
			self["movex"].setText(_("%10d sec") % self.cnt)
		else:
			self.instance.setTitle("Schiffe v.%s %10d shots %10d sec" % (__version__, self.moves, self.cnt))
		self.cnt += 1

	# create new game...
	def new_game(self, loadFromFile=False):
		self["message"].setText("Good Luck!")
		self.gameover = False
		self.Focus = 0
